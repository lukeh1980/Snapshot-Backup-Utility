...
if [ ! -d "/opt/sbu/jobs/$NAME" ]; then
	...
	/opt/sbu/source/create-config.sh "${SOURCE}" "${DEST}" $NAME $INTERVAL $RETENTION $AUTOSTART $SETPERMS $SETOWNER $SETGROUP
	...
	if [ "$SETPERMS" != "" ]; then
		if [ "$SETPERMS" == "no-perms" ]; then
			...
			if [ "$SETPERMS" == "copy" ]; then
				...
				SETPERMS=" --chmod=$SETPERMS "
			fi
			if [ "$SETGROUP" != "" ] && [ "$SETOWNER" != "" ]; then
                		if [ "$SETGROUP" == "copy" ] && [ "$SETOWNER" == "copy" ]; then
                        		CHOWN=" -og"
                		else
                        		CHOWN=" -og --chown=$SETOWNER:$SETGROUP "
                		fi
        		fi
		fi
	fi
	echo "rsync -rltD$SETPERMS$CHOWN ${SOURCE}/ ${FULLDEST}${SOURCE}/" >> /var/log/sbu/$NAME/sbulog
	rsync -rltD$SETPERMS$CHOWN "${SOURCE}/" "${FULLDEST}${SOURCE}/" 2>> /var/log/sbu/$NAME/sbulog
