...
echo "# ExcludeFile sets a file with directories to exclude from syncing. Do not include full path of directories or files in the file, use relative paths from source directory." >> /opt/sbu/jobs/$NAME/$NAME.conf
echo "ExcludeFile=$EXCLUDEFILE" >> /opt/sbu/jobs/$NAME/$NAME.conf
echo "" >> /opt/sbu/jobs/$NAME/$NAME.conf
