...
FILEDEST="${DEST}/$NAME/tmp/$NAME-changes"
...
if [ "$FULLSYNC" == "on" ]; then
	...
	RSYNCSTARTED=0
	nohup "$RSYNC_BIN" "${OPTS[@]}" --dry-run --delete --out-format="%f" "${NEWSOURCE}" "${NEWDEST}" > "${FILEDEST}" 2>> /var/log/sbu/$NAME/sbulog &
	while :
	do	
		PID=$(pgrep -f "${RSYNC_BIN} ${OPTS[@]} --dry-run --delete --out-format=%f ${NEWSOURCE} ${NEWDEST}")
		if [ -n "$PID" ]; then
			if [ -s "${FILEDEST}" ]; then
	...
	"$RSYNC_BIN" "${OPTS[@]}" --dry-run --out-format="%f" "${NEWSOURCE}" "${NEWDEST}" > "${FILEDEST}"
